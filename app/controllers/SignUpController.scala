...
class SignUpController @Inject() (
  ...
  def addrole = Action.async { implicit request =>
    Roleform.form.bindFromRequest.fold(
      ...
      data => {
            ...
            Userroles.add(roleuser).map(res =>
               Redirect("/adduser")
             )
  ...
  def adduser = Action.async { implicit request =>
        ...
        Userform.form.bindFromRequest.fold(
          form => Future.successful( Redirect(routes.ApplicationController.adduser())),
          data => {
            val loginInfo = LoginInfo(CredentialsProvider.ID, data.email)
            userService.retrieve(loginInfo).flatMap {
              case Some(user) =>
              Future.successful(Redirect(routes.ApplicationController.adduser()).flashing("error" -> Messages("user.exists")))
              case None =>
              val authInfo = passwordHasher.hash(data.password)
              val user = User(
                userID = UUID.randomUUID(),
              ...
              Future.successful(Redirect(routes.ApplicationController.getlist()))
          }
        }
    )
